...
function getPromiseResult(source) {
  const prom = Promise.resolve(source);
  const get = prom.then(() => 'success').catch(() => 'fail');
  return get;
}
...
function queuPromises(promises) {
  return promises.reduce((promChain, currProm) => {
    return promChain.then(async (val) => {
      const value = await currProm;
      return val + value;
    });
  }, Promise.resolve(''));
}
